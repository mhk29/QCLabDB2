...
<body>
<div align="right">
    <form action="gallery-new.php" method="post" accept-charset="utf-8" enctype="multipart/form-data">
...
<div align="right">
<?php
$sql = "SELECT image_id, nifti_path, z_path, notes FROM images";
...
$s_tags = array('animal','date','dob','sambabrunno','weight','sex','genotype','dwi','gre','study');
$s_tags_show = array('animal','date','dob','brunno','weight','sex','gtype','dwi','gre','study');

echo '<PRE>';
echo "id"."\t";
foreach ($s_tags_show as $s_tag) {
    echo $s_tag."\t";
}
echo "dwnld";
echo "<br />";
...
while ($row = mysqli_fetch_array($result)) {
    ...
    while ($tag = mysqli_fetch_array($result_tag)) {
        ...
        $mytags[$tag['attribute']] = $tag['value'];
        $modified[$tag['attribute']] = 1;
    ...
    if($row['nifti_path'] == "") echo "bad";
    echo "<a href='".$row['nifti_path']."' download>".$row['image_id']."</a> \t";
